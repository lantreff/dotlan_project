...
$sql_5_orga1 = mysql_query("SELECT * FROM `project_todo` WHERE bearbeiter = '".$mail_orga['user_id']."' AND gruppe = 0 AND event_id = '".$event_id."' AND prio = 5 AND status <> 11 ");
	...
	if(mysql_num_rows($sql_3_orga)>0) { ECHO "OUT_3 ".send_mail_table($sql_3_orga,$sql_3_orga1)."<br>"; }
	if(mysql_num_rows($sql_4_orga)>0) { ECHO "OUT_4 ".send_mail_table($sql_4_orga,$sql_4_orga1)."<br>"; }
	if(mysql_num_rows($sql_5_orga)>0) { ECHO "OUT_5 ".send_mail_table($sql_5_orga,$sql_5_orga1)."<br>"; }
...
$sql_5_gruppe1 = mysql_query("SELECT * FROM `project_todo` WHERE gruppe = '".$mail_gruppen['id']."' AND event_id = '".$event_id."' AND prio = 5 AND status <> 11 ");
	...
	if(mysql_num_rows($sql_3_gruppe)>0) { ECHO "OUT_3 ".send_mail_table($sql_3_gruppe,$sql_3_gruppe1)."<br>"; }
	if(mysql_num_rows($sql_4_gruppe)>0) { ECHO "OUT_4 ".send_mail_table($sql_4_gruppe,$sql_4_gruppe1)."<br>"; }
	if(mysql_num_rows($sql_5_gruppe)>0) { ECHO "OUT_5 ".send_mail_table($sql_5_gruppe,$sql_5_gruppe1)."<br>"; }
