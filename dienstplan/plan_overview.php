...
$query = $DB->query("SELECT id,nick,vorname,nachname FROM user");
...
$plan = array();
while($row = $DB->fetch_array($query)) $plan[] = $row["plan_name"];
...
foreach($plan as $p){
  ...
  if(mysql_result($DB->query("SELECT doppelt_erlaubt FROM project_dienstplan WHERE event_id = '".$event_id."' AND plan_name = '$p'"),0,"doppelt_erlaubt") == 1) continue; 
  $count_gesamt = 0;
  $count_vergeben = 0;
  $count_offen = 0;
  $query = $DB->query("SELECT * FROM project_dienstplan WHERE event_id = '".$event_id."' AND plan_name = '$p'");
  while($row = $DB->fetch_array($query)){
    foreach($fields as $f){
