...
//$projekt_von = @mysql_result($DB->query("SELECT UNIX_TIMESTAMP(von) as von FROM projekte WHERE id = '".$event_id."' LIMIT 1"),0,"von");
for($i=0;$i<24;$i++){
...
global $global;
      ...
      $plan .= " <a href='https://www.google.com/calendar/render?action=TEMPLATE&text=".$row["plan_name"]."&dates=$von/$bis&sprop=website:".$_SERVER["SERVER_NAME"]."&trp=true&location=".$global['sitename']."' target='_blank'><img align='right' border=0 src='../images/16/google_calendar.png' height='13' title='Zu google-Kalender hinzuf&uuml;gen'></a>";
      $plan .= "<br>";
    ...
    if(in_array($std1,$anwesend[$tag])) $color_anwesend = "#00AA00";
