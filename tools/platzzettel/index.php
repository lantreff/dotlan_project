...
foreach($bloecke as $b) for($i=1;$i<=24;$i++) $sitze["$b-".str_pad($i,2,"0",STR_PAD_LEFT)] = array(); // User Bloecke
for($i=1;$i<=18;$i++) $sitze["V-".str_pad($i,2,"0",STR_PAD_LEFT)] = array(); // VIP
...
while($event_user = $DB->fetch_array($sql_event_user)){
  ...
  preg_match("/([A-Z]-[0-9]+)/",$event_user["sitz_nr"],$matches);
  $platz = $matches[1];

  $sitze[$platz]["nick"] = $user["nick"];
  $sitze[$platz]["vorname"] = $user["vorname"];
  $sitze[$platz]["nachname"] = $user["nachname"];
}
