...
if(!$DARF["freeze"]) $PAGE->error_die($HTML->gettemplate("error_nopermission"));
...
if($_POST["submit"]){
  ...
  if($_POST["projekt"] == 1){
    $DB->query("DELETE FROM project_rights_user_rights WHERE right_id IN (SELECT id FROM project_rights_rights WHERE recht NOT IN ('view', 'freeze') AND bereich <> 'rechteverwaltung')");
